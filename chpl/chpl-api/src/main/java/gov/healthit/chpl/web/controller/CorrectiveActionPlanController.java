...
public class CorrectiveActionPlanController {
    ...
    @Autowired
    CorrectiveActionPlanManager capManager;
    @Autowired
    CertifiedProductManager productManager;
    @Autowired
    CertifiedProductDetailsManager productDetailsManager;
    @Autowired
    ...
    public @ResponseBody CorrectiveActionPlanDetails update(
            ...
            EntityRetrievalException, JsonProcessingException, InvalidArgumentsException {
        ...
        List<CorrectiveActionPlanCertificationResultDTO> existingCerts = capManager
                .getCertificationsForPlan(existingPlan.getId());
        ...
        CertifiedProductSearchDetails cpSearchDetails = productDetailsManager
                .getCertifiedProductDetails(after.getCertifiedProductId());
        activityManager.addActivity(ActivityConcept.ACTIVITY_CONCEPT_CERTIFIED_PRODUCT, updateRequest.getId(),
                "A corrective action plan for certified product " + cpSearchDetails.getChplProductNumber()
                        + " was updated.",
                before, after);
    ...
    public @ResponseBody String upload(@PathVariable("capId") Long correctiveActionPlanId,
            ...
            throws InvalidArgumentsException, MaxUploadSizeExceededException, Exception {
        if (file.isEmpty()) {
    ...
    public @ResponseBody CorrectiveActionPlanDetails create(
        ...
        // get the acb that owns the product to make sure we have permissions to
        // create it
        ...
        CertifiedProductSearchDetails cpSearchDetails = productDetailsManager
                .getCertifiedProductDetails(result.getCertifiedProductId());
    ...
    public String deleteAcb(@PathVariable("planId") Long planId) throws JsonProcessingException,
        ...
        CertifiedProductSearchDetails cpSearchDetails = productDetailsManager
                .getCertifiedProductDetails(before.getCertifiedProductId());
    ...
    public String deleteDocumentationById(@PathVariable("docId") Long docId) throws JsonProcessingException,
        ...
        CertifiedProductSearchDetails cpSearchDetails = productDetailsManager
                .getCertifiedProductDetails(before.getCertifiedProductId());
        activityManager.addActivity(ActivityConcept.ACTIVITY_CONCEPT_CERTIFIED_PRODUCT, before.getId(),
                "Documentation was removed from a corrective action plan for certified product "
                        + cpSearchDetails.getChplProductNumber(),
                before, after);
    ...
    private List<String> validateCAP(CorrectiveActionPlanDetails cap) {
        ...
        // surveillance start date, surveillance result, and noncompliance date
        // are always required
        ...
        if (cap.getSurveillanceEndDate() != null) {
            ...
            for (CorrectiveActionPlanCertificationResult criteria : cap.getCertifications()) {
                if (StringUtils.isEmpty(criteria.getDeveloperSummary()) || StringUtils.isEmpty(criteria.getResolution())
            ...
            if (missingField) {
                errors.add(
                        "When surveillance end date is entered, all fields for non-compliant criteria are required.");
            }

            // if surveillance result is true, then the num sites passed/total
            // are required
            ...
            if (cap.getRandomizedSurveillance() == Boolean.TRUE) {
                ...
                if (missingSiteInfo) {
                    errors.add(
                            "Since 'randomized surveillance' is true, the number of sites that passed and total number of sites are required.");
                }
            }
        }

...
# 15 5 * * * cd /some/directory/chpl-api/chpl/chpl-service && ./generateListingResources.sh 2014
# This will run it at 0515 UTC, which (depending on DST) is 0015 EST

# create timestamp and filename
TIMESTAMP=$(date "+%Y.%m.%d-%H.%M.%S")
log=logs/log.generateListingResources.$TIMESTAMP.txt
...
echo "####################################" >> $log
java -cp target/chpl-service-jar-with-dependencies.jar gov.healthit.chpl.app.resource.ListingsByEditionResourceCreatorApp $1 2>&1 >> $log
echo "####################################" >> $log

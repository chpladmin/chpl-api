...
# 15 5 * * * cd /some/directory/chpl-api/chpl/chpl-service && ./updateCertificationStatusApp.sh
# This will run it at 0515 UTC, which (depending on DST) is 0015 EST

# create timestamp and filename
TIMESTAMP=$(date "+%Y.%m.%d-%H.%M.%S")
log=logs/log.updateCertificationStatusApp.$TIMESTAMP.txt
...
echo "####################################" >> $log
java -cp target/chpl-service-jar-with-dependencies.jar gov.healthit.chpl.app.UpdateCertificationStatusApp 2>&1 >> $log
echo "####################################" >> $log

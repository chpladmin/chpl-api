...
public class QuarterlyReportManagerTest extends TestCase {
    ...
    @Rollback(true)
    @Transactional
    public void addPrivilegedSurveillanceDataToTwoReports() throws EntityCreationException,
        InvalidArgumentsException, EntityRetrievalException {
        SecurityContextHolder.getContext().setAuthentication(adminUser);
        Long listingId = 1L;
        QuarterlyReportDTO q1Report = createReport(-1L, 2019, 1L);
        //need a relevant surveillance
        Surveillance openSurv =
                createSurveillance(listingId, new Date(q1Report.getStartDate().getTime() + (24*60*60*1000)));

        PrivilegedSurveillanceDTO map = new PrivilegedSurveillanceDTO();
        map.setQuarterlyReport(q1Report);
        map.setCertifiedProductId(listingId);
        map.setId(openSurv.getId());
        map.setK1Reviewed(true);
        map.setGroundsForInitiating("q1 grounds");
        reportManager.createOrUpdateQuarterlyReportSurveillanceMap(map);
        //make sure the privileged data shows up when getting relevant listings
        List<QuarterlyReportRelevantListingDTO> listingsWithRelevantSurveillance =
                reportManager.getListingsWithRelevantSurveillance(q1Report);
        assertNotNull(listingsWithRelevantSurveillance);
        assertTrue(listingsWithRelevantSurveillance.size() == 1);
        boolean foundListing = false;
        for (QuarterlyReportRelevantListingDTO relevantListing : listingsWithRelevantSurveillance) {
            if (relevantListing.getId().longValue() == listingId.longValue()) {
                foundListing = true;
                assertNotNull(relevantListing.getSurveillances());
                assertEquals(1, relevantListing.getSurveillances().size());
                PrivilegedSurveillanceDTO surv = relevantListing.getSurveillances().get(0);
                assertTrue(surv.getK1Reviewed());
                assertEquals("q1 grounds", surv.getGroundsForInitiating());
            }
        }
        assertTrue(foundListing);

        //add different priv surv data for q2 but same surveillance
        QuarterlyReportDTO q2Report = createReport(-1L, 2019, 2L);
        PrivilegedSurveillanceDTO q2PrivSurv = new PrivilegedSurveillanceDTO();
        q2PrivSurv.setQuarterlyReport(q2Report);
        q2PrivSurv.setCertifiedProductId(listingId);
        q2PrivSurv.setId(openSurv.getId());
        q2PrivSurv.setGroundsForInitiating("q2 grounds");
        reportManager.createOrUpdateQuarterlyReportSurveillanceMap(q2PrivSurv);
        //make sure the privileged data shows up when getting relevant listings
        listingsWithRelevantSurveillance =
                reportManager.getListingsWithRelevantSurveillance(q2Report);
        assertNotNull(listingsWithRelevantSurveillance);
        assertEquals(1, listingsWithRelevantSurveillance.size());
        foundListing = false;
        for (QuarterlyReportRelevantListingDTO relevantListing : listingsWithRelevantSurveillance) {
            if (relevantListing.getId().longValue() == listingId.longValue()) {
                foundListing = true;
                ...
                PrivilegedSurveillanceDTO surv = relevantListing.getSurveillances().get(0);
                assertNotNull(surv);
                assertNull(surv.getK1Reviewed());
                assertEquals("q2 grounds", surv.getGroundsForInitiating());
            }
        }
        assertTrue(foundListing);
        SecurityContextHolder.getContext().setAuthentication(null);

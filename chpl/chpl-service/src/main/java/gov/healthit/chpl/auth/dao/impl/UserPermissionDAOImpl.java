...
public class UserPermissionDAOImpl extends BaseDAOImpl implements UserPermissionDAO {
	...
	public void delete(String authority) {
		...
		Query query = entityManager
				.createQuery("UPDATE UserPermissionEntity SET deleted = true WHERE authority = :authority");
	...
	public void delete(Long permissionId) {
		...
		Query query = entityManager.createQuery(
				"UPDATE UserPermissionEntity SET deleted = true WHERE user_permission_id = :user_permission_id");
	...
	@Override
	@Transactional
	public void deleteMapping(String userName, String authority)
		...
		Query query = entityManager.createQuery(
				"UPDATE UserPermissionUserMappingEntity SET deleted = true WHERE user_id = :userid AND user_permission_id_user_permission = :permissionid");
	...
	public void deleteMappingsForUser(String userName) throws UserRetrievalException {
		...
		UserDTO user = this.userDAO.getByName(userName);
		Query query = entityManager
				.createQuery("UPDATE UserPermissionUserMappingEntity SET deleted = true WHERE c.user_id = :userid");
		query.setParameter("userid", user.getId());
		query.executeUpdate();
	...
	public void deleteMappingsForUser(Long userId) {
		...
		Query query = entityManager
				.createQuery("UPDATE UserPermissionUserMappingEntity SET deleted = true WHERE user_id = :userid");
	...
	public void deleteMappingsForPermission(UserPermissionDTO userPermission) throws UserPermissionRetrievalException {
		...
		Query query = entityManager.createQuery(
				"UPDATE UserPermissionUserMappingEntity SET deleted = true WHERE user_permission_id_user_permission = :permissionid");
	...
	public Set<UserPermissionDTO> findPermissionsForUser(Long userId) {
		...
		Query query = entityManager.createQuery(
				"FROM UserPermissionUserMappingEntity pme JOIN FETCH pme.permission WHERE pme.deleted <> true AND user_id = :userid");
		query.setParameter("userid", userId);
	...
	private void createMapping(UserPermissionUserMappingEntity mapping) throws UserPermissionRetrievalException {
		UserPermissionUserMappingEntity existingMap = getMapping(mapping.getUser().getId(),
	...
	private void createMapping(UserEntity user, UserPermissionEntity permission)
		...
		permissionMapping.setLastModifiedUser(Util.getAuditId());
	...
	private UserPermissionUserMappingEntity getMapping(Long userId, Long permissionId) {
		...
		Query query = entityManager.createQuery(
				"SELECT e FROM UserPermissionUserMappingEntity e "
						+ "WHERE user_id = :userid AND user_permission_id_user_permission = :permissionid",
				UserPermissionUserMappingEntity.class);
		query.setParameter("userid", userId);
		query.setParameter("permissionid", permissionId);
	...
	private UserPermissionEntity getPermissionEntityFromAuthority(String authority)
			throws UserPermissionRetrievalException {

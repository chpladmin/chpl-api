	...
	public static void main(String[] args) throws Exception {
		...
		if(oncRecipientSubscriptions.size() > 0 || allAcbRecipientSubscriptions.size() > 0){
			...
			for(CertificationBodyDTO acb : acbs) {
				...
				currAcbList.add(acb);
		 		List<RecipientWithSubscriptionsDTO> acbRecipientSubscriptions = app.getNotificationDAO().getAllNotificationMappingsForType(permissions, NotificationTypeConcept.ONC_ACB_WEEKLY_ICS_FAMILY_ERRORS, currAcbList);

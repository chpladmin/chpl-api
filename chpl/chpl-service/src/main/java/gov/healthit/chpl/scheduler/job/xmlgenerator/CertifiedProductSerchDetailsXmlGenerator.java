...
public class CertifiedProductSerchDetailsXmlGenerator extends XmlGenerator {
    ...
    public XMLStreamWriter writeCertifiecProduct(CertifiedProductSearchDetails cp, XMLStreamWriter sw) throws XMLStreamException {
        sw = writeCertifiedProductSearchDetails(cp, "listing", sw);
        return sw;
    }
    
    private XMLStreamWriter writeCertifiedProductSearchDetails(CertifiedProductSearchDetails cp, String rootNodeName, XMLStreamWriter sw) throws XMLStreamException  {
        LOGGER.info("Writing XML output for: " + cp.getId());

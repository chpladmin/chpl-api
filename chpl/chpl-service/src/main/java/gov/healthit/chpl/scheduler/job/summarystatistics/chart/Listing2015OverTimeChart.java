...
public class Listing2015OverTimeChart extends SummaryStatisticChart {

    @Override
    public JFreeChart generate(File csv) throws IOException {
        List<ListingsByEditionOverTime> reportData = getDataFromCsv(csv);

        JFreeChart chart = ChartFactory.createTimeSeriesChart("Total 2015 Listings Over Time", "Date", "Listing Count", createDataSet(reportData));
    ...
    private XYDataset createDataSet(List<ListingsByEditionOverTime> data) {
        ...
        TimeSeries series1 = new TimeSeries("Total 2015 Listings");
        ...
        try (Reader reader = new FileReader(csv);
            ...
            for (CSVRecord csvRecord : csvParser) {
                ...
                ListingsByEditionOverTime data = ListingsByEditionOverTime.builder()
                        ...
                        .listingCount(Long.parseLong(csvRecord.get(LISTING_2015_COLUMN)))
                        .build();
                reportData.add(data);
            }
        }

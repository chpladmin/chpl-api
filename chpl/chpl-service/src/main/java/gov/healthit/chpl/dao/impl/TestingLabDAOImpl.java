...
public class TestingLabDAOImpl extends BaseDAOImpl implements TestingLabDAO {
    ...
    public void delete(Long id) throws EntityRetrievalException {
        ...
        if (toDelete != null) {
            ...
            Query query = entityManager.createQuery(
                    "UPDATE ActivityEntity SET deleted = true WHERE activity_object_id = :atlid AND description NOT LIKE '%Deleted testing lab%' AND description NOT LIKE '%no longer marked as deleted%'");
    ...
    public String getMaxCode() {
        String maxCode = null;
        Query query = entityManager.createQuery(
                "SELECT atl.testingLabCode " + "from TestingLabEntity atl " + "ORDER BY atl.testingLabCode DESC",
                String.class);
    ...
    private List<TestingLabEntity> getAllEntities(boolean showDeleted) {
        ...
        if (showDeleted) {
            result = entityManager
                    .createQuery("SELECT atl from TestingLabEntity atl " + "LEFT OUTER JOIN FETCH atl.address ",
                            TestingLabEntity.class)
                    .getResultList();
        } else {
            result = entityManager.createQuery("SELECT atl from TestingLabEntity atl "
                    + "LEFT OUTER JOIN FETCH atl.address " + "where (NOT atl.deleted = true)", TestingLabEntity.class)
                    .getResultList();
        }
        return result;
    }
    ...
    private TestingLabEntity getEntityByName(String name) {
        ...
        Query query = entityManager
                .createQuery(
                        "SELECT atl from TestingLabEntity atl " + "LEFT OUTER JOIN FETCH atl.address "
                                + "where (NOT atl.deleted = true) " + "AND (atl.name = :name) ",
                        TestingLabEntity.class);

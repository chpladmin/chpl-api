...
public class SurveillanceOversightReportDailyApp extends SurveillanceOversightReportApp {
    ...
    public static void main(String[] args) throws Exception {
        ...
        List<RecipientWithSubscriptionsDTO> oncRecipientSubscriptions = oversightApp.getNotificationDAO()
                .getAllNotificationMappingsForType(permissions,
                        NotificationTypeConcept.ONC_DAILY_SURVEILLANCE_BROKEN_RULES, null);
        List<RecipientWithSubscriptionsDTO> allAcbRecipientSubscriptions = oversightApp.getNotificationDAO()
                .getAllNotificationMappingsForType(permissions,
                        NotificationTypeConcept.ONC_ACB_DAILY_SURVEILLANCE_BROKEN_RULES, acbs);
        if (oncRecipientSubscriptions.size() > 0 || allAcbRecipientSubscriptions.size() > 0) {
            // Get full set of data to send in ONC email
            List<CertifiedProductSearchDetails> allCertifiedProductDetails = oversightApp
                    .getAllCertifiedProductSearchDetails();
            ...
            Map<CertificationBodyDTO, CertifiedProductDownloadResponse> certificationDownloadMap = oversightApp
            ...
            for (CertificationBodyDTO acb : acbs) {
                ...
                currAcbList.add(acb);
                List<RecipientWithSubscriptionsDTO> acbRecipientSubscriptions = oversightApp.getNotificationDAO()
                        .getAllNotificationMappingsForType(permissions,
                                NotificationTypeConcept.ONC_ACB_DAILY_SURVEILLANCE_BROKEN_RULES, currAcbList);
                oversightApp.sendAcbDailyEmail(acb, acbRecipientSubscriptions, downloadFolder,
                        certificationDownloadMap);
            }
        }
    ...
    protected void initiateSpringBeans(AbstractApplicationContext context) throws IOException {
        ...
        this.setPresenter((SurveillanceOversightNewBrokenRulesCsvPresenter) context
                .getBean("surveillanceOversightNewBrokenRulesCsvPresenter"));
     ...
     */
    private void sendOncDailyEmail(List<RecipientWithSubscriptionsDTO> oncRecipientSubscriptions, File downloadFolder,
        ...
        File surveillanceReportFile = new File(
                downloadFolder.getAbsolutePath() + File.separator + surveillanceReportFilename);
    ...
    /**
     * send an email to the appropriate recipients with broken surveillance
     * rules for listings within a given ACB
     * 
     * @param acb
     ...
     * @throws MessagingException
     */
    private void sendAcbDailyEmail(CertificationBodyDTO acb,

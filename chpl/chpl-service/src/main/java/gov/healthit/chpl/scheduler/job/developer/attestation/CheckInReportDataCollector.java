...
public class CheckInReportDataCollector {
	...
	private Set<String> activeStatuses = Stream.of(CertificationStatusType.Active.getName(),
			CertificationStatusType.SuspendedByAcb.getName(), CertificationStatusType.SuspendedByOnc.getName())
	...
	public CheckInReportDataCollector(AttestationManager attestationManager,
			...
			ListingSearchService listingSearchService, DirectReviewSearchService directReviewSearchService,
		...
		assurancesCriteria = Arrays.asList(assurancesCriteriaKeys).stream()
				.map(key -> certificationCriterionService.get(key)).collect(Collectors.toList());

		apiCriteria = Arrays.asList(apiCriteriaKeys).stream().map(key -> certificationCriterionService.get(key))
	...
	private CheckInReport getCheckInReport(Developer developer) {
		CheckInAttestation checkInAttestation = checkInReportSourceService.getCheckinReport(developer,
				attestationManager.getMostRecentPastAttestationPeriod());
	...
	private CheckInReport convert(Developer developer) {
		List<CertificationBody> acbs = developerCertificationBodyMapDAO
				.getCertificationBodiesForDeveloper(developer.getId());
		return CheckInReport.builder().developerName(developer.getName()).developerCode(developer.getDeveloperCode())
				.developerId(developer.getId()).published(false)
	...
	private CheckInReport convert(ChangeRequest cr) {
		...
		CheckInReport checkInReport = convert(cr.getDeveloper(), crAttestation.getForm(),
				crAttestation.getAttestationPeriod().getId());
		...
		checkInReport.setRelevantAcbs(
				cr.getCertificationBodies().stream().map(acb -> acb.getName()).collect(Collectors.joining("; ")));
	...
	private CheckInReport convert(Developer developer, AttestationSubmission attestation) {
		...
		CheckInReport checkInReport = convert(developer, attestation.getForm(),
				attestation.getAttestationPeriod().getId());
	...
	private CheckInReport convert(Developer developer, Form form, Long attestationPeriod) {
		return CheckInReport.builder().developerName(developer.getName()).developerCode(developer.getDeveloperCode())
				...
				.informationBlockingResponse(getAttestationResponse(form,
						AttestatationFormMetaData.getInformationBlockingConditionId(attestationPeriod)))
				.informationBlockingNoncompliantResponse(getAttestationOptionalResponse(form,
						AttestatationFormMetaData.getInformationBlockingConditionId(attestationPeriod)))
				.assurancesResponse(getAttestationResponse(form,
						AttestatationFormMetaData.getAssurancesConditionId(attestationPeriod)))
				.assurancesNoncompliantResponse(getAttestationOptionalResponse(form,
						AttestatationFormMetaData.getAssurancesConditionId(attestationPeriod)))
				.communicationsResponse(getAttestationResponse(form,
						AttestatationFormMetaData.getCommunicationConditionId(attestationPeriod)))
				.communicationsNoncompliantResponse(getAttestationOptionalResponse(form,
						AttestatationFormMetaData.getCommunicationConditionId(attestationPeriod)))
				.rwtResponse(
						getAttestationResponse(form, AttestatationFormMetaData.getRwtConditionId(attestationPeriod)))
				.rwtNoncompliantResponse(getAttestationOptionalResponse(form,
						AttestatationFormMetaData.getRwtConditionId(attestationPeriod)))
				.apiResponse(
						getAttestationResponse(form, AttestatationFormMetaData.getApiConditionId(attestationPeriod)))
				.apiNoncompliantResponse(getAttestationOptionalResponse(form,
						AttestatationFormMetaData.getApiConditionId(attestationPeriod)))
				...
				.apiValidation(getApiValidation(developer, LOGGER)).build();
	}
	...
	private List<Developer> getDevelopersActiveListingsDuringMostRecentPastAttestationPeriod() {
		...
		return developerAttestationPeriodCalculator
				.getDevelopersWithActiveListingsDuringAttestationPeriod(mostRecentPastPeriod, LOGGER);
	}
	...
	private Long getTotalSurveillances(Developer developer, Logger logger) {
		return getActiveListingDataForDeveloper(developer, logger).stream()
				.map(listing -> addSurveillanceCount(listing)).collect(Collectors.summingLong(Long::longValue));
	}
	...
	private Long getTotalDirectReviewNonconformities(Developer developer, Logger logger) {
		return directReviewSearchService.getDeveloperDirectReviews(developer.getId(), logger).stream()
				.flatMap(dr -> dr.getNonConformities().stream()).count();
	}
	...
	private Long addOpenAndClosedNonconformityCount(ListingSearchResult listing) {
		Long closed = listing.getClosedSurveillanceNonConformityCount() != null
				? listing.getClosedSurveillanceNonConformityCount()
				: 0L;
		Long open = listing.getOpenSurveillanceNonConformityCount() != null
				? listing.getOpenSurveillanceNonConformityCount()
				: 0L;
		return closed + open;
	}
	...
	private List<ListingSearchResult> getActiveListingDataForDeveloper(Developer developer, Logger logger) {
		if (developerListings.get(developer.getId()) != null) {
			...
			SearchRequest searchRequest = SearchRequest.builder()
					.certificationEditions(Stream.of(CertificationEditionConcept.CERTIFICATION_EDITION_2015.getYear())
							.collect(Collectors.toSet()))
					.developerId(developer.getId()).certificationStatuses(activeStatuses).pageSize(MAX_PAGE_SIZE)
					.pageNumber(0).build();
	...
	private List<ListingSearchResult> getActiveListingDataWithAnyCriteriaForDeveloper(Developer developer,
			List<CertificationCriterion> criteria, Logger logger) {
		SearchRequest searchRequest = SearchRequest.builder()
				.certificationEditions(Stream.of(CertificationEditionConcept.CERTIFICATION_EDITION_2015.getYear())
						.collect(Collectors.toSet()))
				.developerId(developer.getId()).certificationStatuses(activeStatuses)
				.certificationCriteriaIds(
						criteria.stream().map(criterion -> criterion.getId()).collect(Collectors.toSet()))
				.certificationCriteriaOperator(SearchSetOperator.OR).pageSize(MAX_PAGE_SIZE).pageNumber(0).build();
	...
	private List<ListingSearchResult> getAllPagesOfSearchResults(SearchRequest searchRequest, Logger logger) {
		...
		try {
			...
			logger.info("Found {} total listings for developer {}.", searchResults.size(),
					searchRequest.getDeveloperId());
	...
	private String getRealWorldTestingValidation(Developer developer, Logger logger) {
		List<ListingSearchResult> apiEligibleListings = getActiveListingDataWithAnyCriteriaForDeveloper(developer,
				rwtCriteria, logger);
	...
	private String getAssurancesValidation(Developer developer, Logger logger) {
		List<ListingSearchResult> assurancesEligibleListings = getActiveListingDataWithAnyCriteriaForDeveloper(
				developer, assurancesCriteria, logger);

...
public class ActivityConceptDAOImpl extends BaseDAOImpl implements ActivityConceptDAO {
	...
	@Override
	public ActivityConceptDTO create(ActivityConceptDTO dto) throws EntityCreationException,
	...
	private ActivityConceptEntity getEntityById(Long id) throws EntityRetrievalException {
		...
		Query query = entityManager.createQuery( "from ActivityClassEntity where (NOT deleted = true) AND (additional_software_id = :entityid) ", ActivityConceptEntity.class );
	...
	private ActivityConceptEntity getEntityByName(String name) {
		...
		Query query = entityManager.createQuery( "from ActivityClassEntity where (NOT deleted = true) AND (name = :name) ", ActivityConceptEntity.class );
	...
	private List<ActivityConceptEntity> getEntitiesByCertifiedProductId(Long certifiedProductId) {
		...
		Query query = entityManager.createQuery( "from ActivityClassEntity where (NOT deleted = true) AND (certified_product_id = :entityid) ", ActivityConceptEntity.class );
	...
	private List<ActivityConceptEntity> getAllEntities() {
		...
		List<ActivityConceptEntity> result = entityManager.createQuery( "from ActivityClassEntity where (NOT deleted = true) ", ActivityConceptEntity.class).getResultList();
		return result;
	}

...
public class CorrectiveActionPlanCertificationResultDAOImpl extends BaseDAOImpl
        implements CorrectiveActionPlanCertificationResultDAO {
    ...
    @Override
    public CorrectiveActionPlanCertificationResultDTO create(CorrectiveActionPlanCertificationResultDTO toCreate)
        ...
        if (entity != null) {
            ...
            if (cert == null) {
                throw new EntityCreationException(
                        "Cannot find a certification criterion with id " + toCreate.getCertCriterion().getId()
                                + " or number " + toCreate.getCertCriterion().getNumber());
            }
    ...
    @Override
    public CorrectiveActionPlanCertificationResultDTO update(CorrectiveActionPlanCertificationResultDTO toUpdate)
    ...
    public List<CorrectiveActionPlanCertificationResultDTO> getAllForCorrectiveActionPlan(Long correctiveActionPlanId) {
        List<CorrectiveActionPlanCertificationEntity> entities = getEntitiesByCorrectiveActionPlan(
    ...
    private CorrectiveActionPlanCertificationEntity getEntityById(Long id) throws EntityRetrievalException {
        ...
        Query query = entityManager.createQuery(
                "from CorrectiveActionPlanCertificationEntity capCert " + "JOIN FETCH capCert.correctiveActionPlan cap "
                        + "JOIN FETCH capCert.certificationCriterion cert "
                        + "where (NOT capCert.deleted = true) AND (capCert.id = :entityid) ",
                CorrectiveActionPlanCertificationEntity.class);
        query.setParameter("entityid", id);
        List<CorrectiveActionPlanCertificationEntity> result = query.getResultList();

        if (result.size() > 1) {
            throw new EntityRetrievalException(
                    "Data error. Duplicate corrective action plan certification id in database.");
        }
    ...
    private List<CorrectiveActionPlanCertificationEntity> getEntitiesByCorrectiveActionPlan(Long id) {
        ...
        Query query = entityManager.createQuery(
                "from CorrectiveActionPlanCertificationEntity capCert " + "JOIN FETCH capCert.correctiveActionPlan cap "
                        + "JOIN FETCH capCert.certificationCriterion cert "
                        + "where (NOT capCert.deleted = true) AND (cap.id = :corrective_action_plan_id) ",
                CorrectiveActionPlanCertificationEntity.class);

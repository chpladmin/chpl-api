...
public class CorrectiveActionPlanDocumentationDAOImpl extends BaseDAOImpl
        implements CorrectiveActionPlanDocumentationDAO {
    ...
    public List<CorrectiveActionPlanDocumentationDTO> getAllForCorrectiveActionPlan(Long correctiveActionPlanId) {
        List<CorrectiveActionPlanDocumentationEntity> entities = getEntitiesByCorrectiveActionPlan(
    ...
    private CorrectiveActionPlanDocumentationEntity getEntityById(Long id) throws EntityRetrievalException {
        ...
        Query query = entityManager.createQuery(
                "from CorrectiveActionPlanDocumentationEntity capDoc " + "JOIN FETCH capDoc.correctiveActionPlan cap "
                        + "where (NOT capDoc.deleted = true) AND (capDoc.id = :entityid) ",
                CorrectiveActionPlanDocumentationEntity.class);
        query.setParameter("entityid", id);
        List<CorrectiveActionPlanDocumentationEntity> result = query.getResultList();

        if (result.size() > 1) {
            throw new EntityRetrievalException(
                    "Data error. Duplicate corrective action plan documentation id in database.");
        }
    ...
    private List<CorrectiveActionPlanDocumentationEntity> getEntitiesByCorrectiveActionPlan(Long id) {
        ...
        Query query = entityManager.createQuery(
                "from CorrectiveActionPlanDocumentationEntity capDoc " + "JOIN FETCH capDoc.correctiveActionPlan cap "
                        + "where (NOT capDoc.deleted = true) AND (cap.id = :corrective_action_plan_id) ",
                CorrectiveActionPlanDocumentationEntity.class);

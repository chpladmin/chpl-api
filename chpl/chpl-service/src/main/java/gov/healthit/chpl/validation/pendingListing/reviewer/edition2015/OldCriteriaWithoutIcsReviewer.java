...
public class OldCriteriaWithoutIcsReviewer implements Reviewer {
    ...
    private String checkForError(PendingCertifiedProductDTO listing, String criteria, long criteriaId, long relevantDate) {
    ...
    private boolean hasRelevantCriteria(PendingCertifiedProductDTO listing, long criteriaId) {
        return listing.getCertificationCriterion().stream()
                .anyMatch(result -> result.getCriterion().getId().equals(criteriaId)
                        && result.getMeetsCriteria());
    }

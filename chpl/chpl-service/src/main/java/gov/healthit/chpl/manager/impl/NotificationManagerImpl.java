...
public class NotificationManagerImpl implements NotificationManager {
    ...
    public NotificationTypeRecipientMapDTO addRecipientNotificationMap(NotificationTypeRecipientMapDTO mapping)
        ...
        if (!notificationDao.hasNotificationType(mapping.getSubscription().getNotificationType(),
                Util.getCurrentUser().getPermissions())) {
            throw new AccessDeniedException(
                    "User " + Util.getUsername() + " does not have permission to create notification with type "
                            + mapping.getSubscription().getNotificationType().getName());
        }
        RecipientDTO recipient = mapping.getRecipient();
        // if no id is passed in, look for an existing recipient with the same
        // email
        // to avoid duplicates
        if (recipient != null && recipient.getId() == null && !StringUtils.isBlank(recipient.getEmailAddress())) {
            String email = recipient.getEmailAddress().trim();
            LOGGER.debug("Looking for existing recipient with email address " + email);
            recipient = notificationDao.findRecipientByEmail(email);
        }

        NotificationTypeRecipientMapDTO result = notificationDao.createNotificationMapping(recipient,
                mapping.getSubscription().getNotificationType(), mapping.getSubscription().getAcb());
        return result;
    }
    ...
    public void deleteRecipientNotificationMap(NotificationTypeRecipientMapDTO mapping) {
        ...
        notificationDao.deleteNotificationMapping(mapping.getRecipient(),
                mapping.getSubscription().getNotificationType(), mapping.getSubscription().getAcb());
    }

    /**
     * Deletes the recipient as far as the current user would be aware of. If
     * the recipient has subscriptions that the current user does not have
     * access to, leave those and leave the recipient but delete any
     * subscriptions that the current user does have access to. If the recipient
     * is not associated with any other subscriptions in the system (regardless
     * of current user permissions) delete the recipient email address as well.
     * 
     * @param recipientId
     */
    @PreAuthorize("hasAnyRole('ROLE_ADMIN', 'ROLE_ACB')")
    ...
    public void deleteRecipient(Long recipientId) throws EntityRetrievalException {
        ...
        // delete those
        RecipientWithSubscriptionsDTO fullRecip = notificationDao.getAllNotificationMappingsForRecipient(recipientId,
                Util.getCurrentUser().getPermissions(), acbs);
        for (SubscriptionDTO sub : fullRecip.getSubscriptions()) {
            // if the recip has no other subscriptions, dao will take care of
            // deleting it

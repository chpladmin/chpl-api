...
public class CertifiedProductValidatorFactoryImpl implements CertifiedProductValidatorFactory {
    ...
    @Autowired
    private CertifiedProductAllowedValidator allowedValidator;
    @Autowired
    private AmbulatoryComplete2014Validator ambulatoryComplete2014Validator;
    @Autowired
    private AmbulatoryModular2014Validator ambulatoryModular2014Validator;
    @Autowired
    private InpatientComplete2014Validator inpatientComplete2014Validator;
    @Autowired
    private InpatientModular2014Validator inpatientModular2014Validator;
    @Autowired
    private CertifiedProduct2015Validator cp2015Validator;
            ...
            if (product.getPracticeType().equalsIgnoreCase(PRACTICE_TYPE_AMBULATORY)) {
                if (product.getProductClassificationName().equalsIgnoreCase(PRODUCT_CLASSIFICATION_MODULAR)) {
                    ...
                    product.getErrorMessages().add(
                            "Cannot determine if the 2014 Ambulatory product is Modular or Complete so it cannot be validated.");
                    LOGGER.error("Cannot find validator for 2014, ambulatory, and classification '"
                            + product.getProductClassificationName() + "'.");
                }
                ...
                if (product.getProductClassificationName().equalsIgnoreCase(PRODUCT_CLASSIFICATION_MODULAR)) {
                    ...
                    product.getErrorMessages().add(
                            "Cannot determine if the 2014 Inpatient product is Modular or Complete so it cannot be validated.");
                    LOGGER.error("Cannot find validator for 2014, inpatient, and classification '"
                            + product.getProductClassificationName() + "'.");
                }
            } else {
                product.getErrorMessages()
                        .add("This 2014 product is neither Ambulatory nor Inpatient so it cannot be validated.");
    ...
    public CertifiedProductValidator getValidator(CertifiedProductSearchDetails product) {
        ...
        if (productCertificationEdition != null && productCertificationEdition.equals("2014")) {
            ...
            if (StringUtils.isEmpty(practiceTypeName) || StringUtils.isEmpty(productClassificationName)) {
                product.getErrorMessages()
                        .add("Cannot determine if this product is Ambulatory or Inpatient so it cannot be validated.");
                LOGGER.error(
                        "Cannot validate a product that doesn't have practice type and/or classification specified.");
                return null;
            }
            ...
            if (practiceTypeName.equalsIgnoreCase(PRACTICE_TYPE_AMBULATORY)) {
                ...
                product.getErrorMessages()
                        .add("This 2014 product is not Inpatient or Ambulatory so it cannot be validated.");
                LOGGER.error("Cannot find validator for practice type '" + product.getPracticeType() + "'");
            }

...
public class NotificationDAOImpl extends BaseDAOImpl implements NotificationDAO {
    ...
    public boolean hasNotificationType(NotificationTypeDTO type, Set<GrantedPermission> permissions) {
        ...
        Query query = entityManager.createQuery("SELECT DISTINCT nt " + "FROM NotificationTypeEntity nt "
                + "LEFT OUTER JOIN FETCH nt.permissions perms " + "LEFT OUTER JOIN FETCH perms.permission perm "
                + "WHERE nt.deleted <> true " + "AND perm.authority IN (:authorities) " + "AND nt.id = :typeId ",
                NotificationTypeEntity.class);
    ...
    public List<NotificationTypeDTO> getAllNotificationTypes(Set<GrantedPermission> permissions) {
        ...
        String hql = "SELECT DISTINCT nt " + "FROM NotificationTypeEntity nt "
                + "LEFT OUTER JOIN FETCH nt.permissions perms " + "LEFT OUTER JOIN FETCH perms.permission perm "
    ...
    public List<RecipientWithSubscriptionsDTO> getAllNotificationMappings(Set<GrantedPermission> permissions,
        ...
        List<RecipientWithSubscriptionsEntity> allMappings = findRecipientsWithNotifications(null, authorityNames, null,
                acbIds);
    ...
    public List<RecipientWithSubscriptionsDTO> getAllNotificationMappingsForType(Set<GrantedPermission> permissions,
        ...
        List<RecipientWithSubscriptionsEntity> allMappings = findRecipientsWithNotifications(null, authorityNames,
                notificationType.getName(), acbIds);
        List<RecipientWithSubscriptionsDTO> results = new ArrayList<RecipientWithSubscriptionsDTO>();
        for (RecipientWithSubscriptionsEntity mapping : allMappings) {
            RecipientWithSubscriptionsDTO dto = new RecipientWithSubscriptionsDTO(mapping);
            results.add(dto);
    ...
    public RecipientWithSubscriptionsDTO getAllNotificationMappingsForRecipient(Long recipientId,
        ...
        List<RecipientWithSubscriptionsEntity> allMappings = findRecipientsWithNotifications(recipientId,
                authorityNames, null, acbIds);
        if (allMappings == null || allMappings.size() == 0) {
            String msg = String.format(messageSource.getMessage(
                    new DefaultMessageSourceResolvable("recipient.notFound"), LocaleContextHolder.getLocale()));
    ...
    public RecipientDTO findRecipientByEmail(String email) throws EntityRetrievalException {
        Query query = entityManager.createQuery("SELECT recip " + "FROM NotificationRecipientEntity recip "
                + "WHERE recip.deleted <> true " + "AND UPPER(recip.email) = :email",
    ...
    public RecipientDTO getRecipientById(Long id) throws EntityRetrievalException {
        Query query = entityManager.createQuery("SELECT recip " + "FROM NotificationRecipientEntity recip "
                + "WHERE recip.deleted <> true " + "AND recip.id = :id", NotificationRecipientEntity.class);
    ...
    public RecipientDTO updateRecipient(RecipientDTO updatedRecipient) throws EntityNotFoundException {
        NotificationRecipientEntity entityToUpdate = entityManager.find(NotificationRecipientEntity.class,
    ...
    public void deleteNotificationMapping(RecipientDTO recipient, NotificationTypeDTO notificationType,
        ...
        List<NotificationTypeRecipientMapEntity> foundMappings = findNotificationMapping(recipient.getId(),
                notificationType.getId(), acbId);
        if (foundMappings != null && foundMappings.size() > 0) {
            ...
            LOGGER.error("Could not find notification-recipient mapping with recipient id " + recipient.getId()
                    + ", notification type id " + notificationType.getId() + ", and acb id " + acbId);
        }

        // if the recipient is not signed up for any more notifications
        // then delete them
        List<NotificationTypeRecipientMapEntity> otherNotificationsForRecip = findNotificationMappingsForRecipient(
                recipient.getId());
        if (otherNotificationsForRecip == null || otherNotificationsForRecip.size() == 0) {
            NotificationRecipientEntity recipToDelete = entityManager.find(NotificationRecipientEntity.class,
                    recipient.getId());
    ...
    private List<RecipientWithSubscriptionsEntity> findRecipientsWithNotifications(Long recipId,
            List<String> authorityNames, String notificationTypeName, List<Long> acbIds) {
        String hql = "SELECT DISTINCT recip " + "FROM RecipientWithSubscriptionsEntity recip "
                + "LEFT OUTER JOIN FETCH recip.subscriptions subs " + "LEFT OUTER JOIN FETCH subs.acb acb "
                + "LEFT OUTER JOIN FETCH subs.notificationType nt " + "LEFT OUTER JOIN FETCH nt.permissions perms "
                + "LEFT OUTER JOIN FETCH perms.permission perm " + "WHERE recip.deleted <> true ";
        if (recipId != null) {
    ...
    private NotificationTypeRecipientMapEntity findNotificationMappingById(Long mappingId) {
        Query query = entityManager.createQuery("SELECT DISTINCT mapping "
                + "FROM NotificationTypeRecipientMapEntity mapping " + "LEFT OUTER JOIN FETCH mapping.recipient "
                + "LEFT OUTER JOIN FETCH mapping.acb " + "LEFT OUTER JOIN FETCH mapping.notificationType "
                + "WHERE mapping.deleted <> true " + "AND mapping.id = :mappingId",
    ...
    private List<NotificationTypeRecipientMapEntity> findNotificationMappingsForRecipient(Long recipientId) {
        Query query = entityManager.createQuery("SELECT DISTINCT recip "
                + "FROM NotificationTypeRecipientMapEntity recip " + "LEFT OUTER JOIN FETCH recip.recipient "
                + "LEFT OUTER JOIN FETCH recip.acb " + "LEFT OUTER JOIN FETCH recip.notificationType "
                + "WHERE recip.deleted <> true " + "AND recip.recipientId = :recipientId",
                NotificationTypeRecipientMapEntity.class);
    ...
    private List<NotificationTypeRecipientMapEntity> findNotificationMapping(Long recipientId, Long notificationTypeId,
            Long acbId) {
        String hql = "SELECT DISTINCT mapping " + "FROM NotificationTypeRecipientMapEntity mapping "
                + "LEFT OUTER JOIN FETCH mapping.recipient recipient " + "LEFT OUTER JOIN FETCH mapping.acb acb "
                + "LEFT OUTER JOIN FETCH mapping.notificationType type " + "WHERE mapping.deleted <> true "

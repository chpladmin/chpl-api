...
public class UpdateParticipantsJob implements Job {
    ...
    public void execute(JobExecutionContext jobContext) throws JobExecutionException {
        ...
        try {
            ...
            List<ListingSearchResult> activeListingsWithG3ConfirmedWithFlexibleUpload = listingSearchService.getAllPagesOfSearchResults(SearchRequest.builder()
                    //TODO: Not 100% sure about filtering by status. Should we fix inactive ones, too?
                    .certificationStatuses(Stream.of(CertificationStatusType.Active.getName(),
                            CertificationStatusType.SuspendedByAcb.getName(),
                            CertificationStatusType.SuspendedByOnc.getName()).collect(Collectors.toSet()))
            ...
            for (ListingSearchResult listing : activeListingsWithG3ConfirmedWithFlexibleUpload) {
                ...
                if (listingUpdateResult.getUpdatedListing() != null) {
                    ...
                    if ((!CollectionUtils.isEmpty(listingUpdateResult.getUpdatedListing().getErrorMessages())
                            || !CollectionUtils.isEmpty(listingUpdateResult.getUpdatedListing().getWarningMessages()))
                         && (isAnyMessageAboutSed(listingUpdateResult.getUpdatedListing().getErrorMessages())
                            || isAnyMessageAboutSed(listingUpdateResult.getUpdatedListing().getWarningMessages()))) {
                        //there will be at least 1 error about duplicate CHPL ID since this is a confirmed listing
                    ...
                    LOGGER.warn("Listing " + listingUpdateResult.getOriginalListing().getId() + " participants could not be updated by any strategy.");
                }
            }
        } catch (Exception ex) {
    ...
    private ListingUpdateResult applyStrategies(CertifiedProductSearchDetails listing) {
        ...
        for (ParticipantUpdateStrategy strategy : participantUpdateStrategies) {
            if (result.getUpdatedListing() == null) {

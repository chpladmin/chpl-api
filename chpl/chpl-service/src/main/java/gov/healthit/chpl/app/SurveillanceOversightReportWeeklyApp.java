...
public class SurveillanceOversightReportWeeklyApp extends SurveillanceOversightReportApp {
    ...
    public static void main(String[] args) throws Exception {
        ...
        List<RecipientWithSubscriptionsDTO> oncRecipientSubscriptions = oversightApp.getNotificationDAO()
                .getAllNotificationMappingsForType(permissions,
                        NotificationTypeConcept.ONC_WEEKLY_SURVEILLANCE_BROKEN_RULES, null);
        List<RecipientWithSubscriptionsDTO> allAcbRecipientSubscriptions = oversightApp.getNotificationDAO()
                .getAllNotificationMappingsForType(permissions,
                        NotificationTypeConcept.ONC_ACB_WEEKLY_SURVEILLANCE_BROKEN_RULES, acbs);
        if (oncRecipientSubscriptions.size() > 0 || allAcbRecipientSubscriptions.size() > 0) {
            ...
            for (CertificationBodyDTO acb : acbs) {
                ...
                List<RecipientWithSubscriptionsDTO> acbRecipientSubscriptions = oversightApp.getNotificationDAO()
                        .getAllNotificationMappingsForType(permissions,
                                NotificationTypeConcept.ONC_ACB_WEEKLY_SURVEILLANCE_BROKEN_RULES, currAcbList);
                oversightApp.sendAcbWeeklyEmail(acb, acbRecipientSubscriptions, downloadFolder,
    ...
    protected void initiateSpringBeans(AbstractApplicationContext context) throws IOException {
        ...
        this.setPresenter((SurveillanceOversightAllBrokenRulesCsvPresenter) context
                .getBean("surveillanceOversightAllBrokenRulesCsvPresenter"));
